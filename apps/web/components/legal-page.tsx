...
export function LegalPage({ locale, settings, kind }: { locale: Locale; settings: Record<string, string>; kind: "privacy" | "terms" }) {
  ...
  return (
    <PublicShell locale={locale} settings={settings}>
      <PageHeader eyebrow={`// ${kind}`} title={title} body={locale === "es" ? "Vigente desde 2026-08-31. Ajustar antes de producción con proveedor SMTP y dominio definitivos." : "Effective 2026-08-31. Update before production with final SMTP provider and domain."} />
