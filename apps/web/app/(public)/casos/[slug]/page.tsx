...
export default async function CaseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [settings, item] = await Promise.all([getSettings(), getCaseBySlug("es", slug)]);
  if (!item) {
    return (
      <PublicShell locale="es" settings={settings}>
        <main className="grid min-h-[calc(100dvh-var(--header-height))] place-items-center px-5 py-20 text-center">
          <div>
            ...
            <p className="mx-auto mt-6 max-w-xl text-[color:var(--muted)]">El caso no existe o todavia no fue publicado.</p>
